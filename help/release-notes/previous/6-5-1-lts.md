...
* **FORMS-24688:** Al configurar JBoss EAP 8.0 en Linux, los scripts de shell modificados en Windows pueden contener finales de línea CRLF, lo que provoca `/bin/sh^M: bad interpreter` o $`\r`: el comando no encontró errores. Convierta los archivos afectados a finales de línea LF usando dos2unix para resolver esto.

* **FORMS-24689:** En el modo de actualización llave en mano, es posible que la actualización del puerto de base de datos en `lc_turnkey.xml` no se aplique correctamente durante la actualización, lo que hace que el proceso haga referencia al valor de puerto antiguo.
