...
>[!IMPORTANT]
>
> **Revisión obligatoria**: para evitar problemas de SNFE (SegmentNotFoundException) con la compactación sin conexión al instalar el Service Pack 1, instale la revisión descrita en [Problemas conocidos: corrupción del repositorio durante la compactación en línea](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
Ya está disponible AEM 6.5 Forms LTS en JEE. Para obtener más información sobre los entornos admitidos, consulte el documento Combinaciones de [plataformas admitidas](/help/forms/using/aem-forms-jee-supported-platforms.md). Los vínculos del instalador están disponibles en la página [Versiones de AEM Forms](https://experienceleague.adobe.com/es/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases).

#### Qué incluye AEM Forms 6.5 LTS SP1

**Actualizaciones de soporte de Java**

Se ha introducido la compatibilidad con las versiones más recientes de Java:

* Oracle Java™ 17/21 (64 bits) (Vea más detalles en la sección [Compatibilidad con Java](#java-support)).
...
Para información detallada sobre la configuración, consulte la guía de instalación y configuración.
...
**Requisito de migración del espacio de nombres**

* Jakarta EE 9 introduce un cambio de espacio de nombres de `javax.*` a `jakarta.*`
* Todos los **DSC personalizados** deben migrarse al espacio de nombres `jakarta.*`
* AEM Forms 6.5 LTS SP1 solo admite **servidores de aplicaciones basados en Jakarta EE 9+**

Para obtener más información, consulte **Migración del espacio de nombres Javax a Jakarta**.

#### Migración del espacio de nombres `javax` a `jakarta`

A partir de **AEM Forms 6.5 LTS SP1**, solo se admiten los servidores de aplicaciones que implementan **Jakarta Servlet API 5/6**. Con **Jakarta EE 9 y versiones posteriores**, todas las API pasaron del espacio de nombres `javax.{}` a `jakarta.`.

Como resultado, **todos los DSC personalizados deben usar el espacio de nombres `jakarta`**. Los componentes personalizados creados con las API `javax.{}` **no son compatibles** con los servidores de aplicaciones admitidos.
...
**Opción 1: Migración de código fuente (recomendada)**
...
* Usar la herramienta **Transformador Eclipse** para convertir binarios compilados (`.jar`, `.war`) de `javax` a `jakarta`
...
A continuación, se muestran ejemplos comunes de cambios en el espacio de nombres necesarios durante la migración:
...
La tabla siguiente muestra los cambios comunes de espacio de nombres necesarios al migrar de `javax` a `jakarta`:
...
Utilice estas asignaciones como referencia al actualizar el código fuente de DSC personalizado o al validar binarios transformados.
...
* Se ha definido la estrategia de mantenimiento y soporte para AEM 6.5. Esta corrección incluye lo siguiente:
...
>[!NOTE]
>
> (Solo Forms) Oracle Java™ 8/11 (64 bits) necesario para una instalación única.
...
**Entorno**
Se aplica a: clientes de AEM 6.5 LTS (On-Premise) que instalan el Service Pack 1 (SP1). SP1 se suministra como un archivo JAR de inicio rápido.
...
* **FORMS-24688:** Al configurar JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) en Linux, los scripts creados o editados en Windows pueden contener extremos de línea CRLF, lo que provoca errores /bin/sh^M: intérprete incorrecto o $&#39;\r&#39;: comando no encontrado. Asegúrese de que los archivos afectados utilizan extremos de línea LF. Esto se puede lograr utilizando herramientas como dos2unix o cualquier método equivalente.

* **FORMS-24689:** En el modo llave en mano, la actualización del puerto de la base de datos no se aplica correctamente durante la instalación nueva o la actualización. En el modo de instalación nueva, los usuarios solo pueden seleccionar entre los puertos mostrados en la pantalla. En el modo de actualización, el nuevo puerto de base de datos debe actualizarse manualmente en lc_turnkey.xml antes de continuar con la actualización.

* **FORMS-24687:** En implementaciones de clúster de AEM Forms 6.5 LTS en JBoss EAP 8, los archivos domain/configuration/domain_oracle.xml, domain_mysql.xml y domain_mssql.xml pueden contener una etiqueta `<security>` duplicada, lo que da como resultado XML no válido que impide que se inicie el controlador de dominio.
...
Los usuarios experimentaban daños en el repositorio durante la compactación en línea si esta se ejecutaba anteriormente en el repositorio JCR. Se puede producir una `SegmentNotFoundException` (SNFE) en este escenario. Puede provocar daños en el repositorio.

Para resolver el problema, instale la revisión a través del Administrador de paquetes desde [Distribución de software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Dado que la revisión incluye un paquete `oak-segment-tar` de bajo nivel, la instancia se reinicia después de la instalación.

Planifique el tiempo de inactividad de una instancia al aplicarla. Para la compactación sin conexión, use el [Jar ejecutado por Oak](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) correspondiente, también disponible en Distribución de software.

>[!NOTE]
>
> Para cualquier operación ejecutada por Oak, use el [Jar ejecutado por Oak 1.78.1-B006](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (enlazado arriba).
...
### Falta el paquete `com.adobe.granite.apicontroller` en AEM 6.5 LTS SP1 (GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

Falta el paquete `com.adobe.granite.apicontroller` en AEM 6.5 LTS SP1. Este paquete controla cómo se resuelven los paquetes OSGi y puede evitar que se resuelvan en otros paquetes, lo que resulta útil para limitar las API expuestas.

Instale la revisión de [Distribución de software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip) para usar esta funcionalidad.
...
* [Lista de paquetes OSGi incluidos en Experience Manager 6.5 LTS, Service Pack 1](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Lista de paquetes de contenido incluidos en Experience Manager 6.5 LTS, Service Pack 1](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
