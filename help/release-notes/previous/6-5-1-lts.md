...
* **FORMS-24689:** En el modo de actualización llave en mano, las actualizaciones en el puerto de base de datos no se seleccionan en la interfaz de usuario y deben modificarse manualmente en el archivo de configuración lc_turnkey.xml. Si no se actualiza correctamente, el sistema puede seguir utilizando el valor del puerto antiguo.

* **FORMS-24687:** En implementaciones de clúster LTS de AEM Forms 6.5.1 en JBoss EAP 8, `domain/configuration/domain_oracle.xml` puede contener una etiqueta `<security>` duplicada, lo que da como resultado XML no válido que impide que se inicie el controlador de dominio.
