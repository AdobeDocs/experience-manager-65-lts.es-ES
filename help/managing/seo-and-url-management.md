---
title: Prácticas recomendadas para la administración de SEO y URL
description: Obtenga información sobre las prácticas recomendadas de SEO y las recomendaciones sobre una implementación AEM.
...
# Prácticas recomendadas para la administración de SEO y URL{#seo-and-url-management-best-practices}

La optimización de los motores de búsqueda (SEO) se ha convertido en un tema de mucha importancia para muchos expertos en marketing. Como resultado, las cuestiones relacionadas con el SEO deben abordarse en muchos proyectos de AEM.

Primero, en este documento se describen algunas [prácticas recomendadas sobre SEO](#seo-best-practices) y recomendaciones sobre una implementación de AEM. Luego, se profundiza en algunos de los [pasos de implementación más complejos](#aem-configurations) que se mencionan en la primera sección.
...
Existen algunas prácticas recomendadas aceptadas con respecto a las URL.

En el proyecto de AEM, al evaluar las URL, pregúntese lo siguiente:

“Si un usuario viese esta URL, pero no el contenido de la página, ¿podría describirla?”
...
En esta sección se describen los pasos de implementación necesarios para la configuración de AEM con las siguientes recomendaciones SEO.
...
La tendencia de los últimos años ha sido eliminar parámetros para facilitar la lectura de las URL. En muchas plataformas, esto implica implementar redirecciones en el servidor web o en la red de distribución de contenido (CDN), pero Sling lo hace sencillo. Selectores de Sling:

* Facilitan la lectura de las URL.
* Permiten almacenar en caché las páginas en Dispatcher y mejorar la seguridad.
* Permiten dirigir el contenido directamente, en lugar de tener un servlet genérico que recupera el contenido. Esto otorga las ventajas de las ACL que aplica a su repositorio y filtros que aplica en Dispatcher.
...
Los **servlets bin** siguen el patrón al que muchos desarrolladores están acostumbrados por la programación J2EE. El servlet se registra en una ruta específica, que en AEM suele estar en `/bin` y extrae los parámetros de solicitud necesarios de la cadena de consulta.
...
En este caso, se puede acceder automáticamente al recurso al que se dirige la URL (una instancia del recurso `myPageType`) en el servlet. Para acceder a él, ejecute lo siguiente:
...
En AEM, todas las páginas web se almacenan en `/content/my-brand/my-content`. Aunque esta ubicación sea útil desde la perspectiva de la administración de datos del repositorio, no tiene por qué ser la forma en la que desea que los clientes vean el sitio. Además, puede entrar en conflicto con las directrices de SEO para que las URL sean lo más cortas posible. Además, es posible que sirva varios sitios web desde la misma instancia de AEM y desde diferentes nombres de dominio.
...
La propiedad `sling:alias` permite disfrutar de todos los beneficios de Adobe. Puede añadir `sling:alias` como propiedad a cualquier recurso para permitir un nombre de alias para el recurso. En el ejemplo anterior, tendría lo siguiente:
...
1. **Asignación de resultados de URL en páginas**
   ...
   Después de haber definido las asignaciones en el Apache Sling Resource Resolver, utilice estas asignaciones en los componentes para asegurarse de que las direcciones URL que se emitan en las páginas sean cortas y ordenadas. Puede hacerlo utilizando la función de asignación del `ResourceResolver`.
...
Los motores de búsqueda *deben* comprobar la presencia de un archivo `robots.txt` en la raíz del sitio antes de rastrear el sitio. Mientras que los principales motores de búsqueda como Google, Yahoo o Bing respetan este archivo, no es el caso de algunos motores de búsqueda extranjeros.
